...
{	    
	...
	class API
	{
		public:				
			static bool Init();
			static void Close();
			static bool LoadScript(std::filesystem::path path, int argc = 0, wchar_t * argv[] = nullptr);
		...
		private:
			...
			static std::unordered_map<type::Type::id_type, type::Type> _registeredTypes;
			static std::unordered_map<std::string, type::Type::id_type> _typeNameToId;
	...
	constinit inline static struct PyMethodDef API_Methods[] = {
		...
		{ "RegisterTransaction", API::RegisterTransaction, METH_VARARGS, "Register transaction" },
		{ "InitTransaction", API::InitTransaction, METH_VARARGS, "InitTransaction" },
	...
	template<>
	struct ParseFormatStr<int>{
		constexpr inline std::string operator()() const {return "i";}
	};

	template<>
	struct ParseFormatStr<std::size_t>{
		constexpr inline std::string operator()() const {return "u";}
	};

	template<>
	struct ParseFormatStr<char *, Py_ssize_t>{
		constexpr inline std::string operator()() const {return "s#";}
	};

	template<class T>
	struct ParseFormatStr<std::optional<T>>{
		constexpr inline std::string operator()() const {return "|" + ParseFormatStr<T>{}();}
	};

	template<class T>
	struct ParseFormatStr<std::vector<T>>{
		constexpr inline std::string operator()() const {return "O";}
	};

	template<class ... Args>
	std::tuple<Args...> ParseArgs(PyObject* args)
	{
		std::string parseStr = ParseFormatStr<Args...>{}();
		
		std::tuple<Args...> retTuple;

		bool ok = std::apply([&args, &parseStr](Args & ... argsBuffer) -> bool {
