...
namespace arte::python
{
	...
	constinit inline static struct PyModuleDef modDef = {
		PyModuleDef_HEAD_INIT, "arte", NULL, -1, API_Methods, 
		NULL, NULL, NULL, NULL
	};

    PyMODINIT_FUNC PyInit_arte(void)
    {
	...
	bool API::OnCall(std::size_t n, CPyObject argList)
    {
        ...
        CPyObject pModulePath = PyUnicode_FromString(std::filesystem::current_path().string().c_str());
        ...
        if (!mainDict) {
            PyErr_Print();
            std::puts("arte::python Failed to access dictionary object from __main__ module\n");
            return false;
        }
        ...
        if (OnCallFunc && PyCallable_Check(OnCallFunc)) {
            ...
            CPyObject result = PyObject_CallFunctionObjArgs(OnCallFunc, *pN, *argList, NULL);
            if (result) {
    ...
    PyObject* API::RegisterTransaction(PyObject* self, PyObject* args)
	{
		...
		//ID, NAME, IN, OUT
		if(!PyArg_ParseTuple(args, "ns#O!s#", &id, &name, &nameLen, &PyList_Type, &insList, &retName, &retNameLen))
        ...
        //[string, ...]
        for(unsigned currentArg = 0; currentArg < PyList_Size(insList); ++currentArg) {
            ...
            {
                std::puts(std::format("arte::python type `{}` not registered", typeName).c_str());
		        return PyLong_FromLong(1);
            }
        ...
        {
            std::puts(std::format("arte::python type `{}` not registered", retName).c_str());
		    return PyLong_FromLong(1);
        }

        _transactionRegistry.Add(id, name, insVec, _typeNameToId.at(retName));
        
        std::puts(std::format("arte::python registered transaction `{}` with return type {}", name, retName).c_str());
    ...
    PyObject* API::RunTransaction(PyObject* self, PyObject* args)
    {
		...
		if(!PyArg_ParseTuple(args, "ns#n|O", &id, &name, &nameLen, &n, &argsObj)){
            Py_DECREF(argsObj);
            return PyLong_FromLong(1);
        }
        ...
        {
            std::puts(std::format("arte::python wrong number of arguments {} for function {}, expected `{}`",
                PyList_Size(argsObj), name, transaction.get().ins.size()).c_str());
            Py_DECREF(argsObj);
            return PyLong_FromLong(1);
        }

        std::puts(std::format("arte::python run transaction `{}`", name).c_str());

        CPyObject pName = PyUnicode_FromString((std::string("scripts.transactions.") + name).c_str());
        if(!pName)
        {
            std::puts(std::format("arte::python error constructing module name `{}`", name).c_str());
            return PyLong_FromLong(1);
        }
        PyObject * pModule = PyImport_Import(*pName);
        if(!pModule)
        {
            std::puts(std::format("arte::python error importing module `{}`", name).c_str());
            return PyLong_FromLong(1);
        }
        ...
        PyObject * pValue = PyObject_CallFunctionObjArgs(pFunc, PyLong_FromSize_t(n), argsObj, NULL);
        ...
        */
        Py_XDECREF(pFunc);
        const auto & returnType = _registeredTypes.at(transaction.get().retT).name;
       	return PyTuple_Pack(2, PyUnicode_FromString(returnType.c_str()), pValue);
    }
}