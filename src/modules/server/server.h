	...
	class Server {
	  public:
			...
			_ioContext(ioContext), _endpoint(tcp::v4(), port)
		{
			if (_running.test_and_set()) {
				std::puts(
					"Server won't start. Weird running flag initialization.");
				return;
			}
	  ...
	  protected:
		awaitable<void> accept(tcp::socket socket)
		{
			...
			std::puts(
				std::format("Connection with {} established",
			...
			auto			   cancelToken = asio::bind_executor(
				  socket.get_executor(),
			...
			try {
				...
				while (connected.test()) {
					...
					std::puts(std::format("echo read {} bytes from socket", n)
								  .c_str());
					...
					for (std::string line; std::getline(input, line);) {
						...
						for (; pos != std::string::npos;
							...
							strs.push_back(
								line.substr(initialPos, pos - initialPos));
							initialPos = pos + 1;
						}
						// Add the last one
						strs.push_back(line.substr(
							initialPos,
							std::min(pos, line.size()) - initialPos + 1));
					}
					if (strs.size() < 2) {
			...
			std::puts(
				std::format("Connection with {} ended",
							socket.remote_endpoint().address().to_string())
					.c_str());
			co_return;
		}
		...
		awaitable<void> listen()
		{
			...
			std::puts(std::format("server started listening on {} with port {}",
			...
			while (_running.test()) {
				...
				_accepted.try_emplace(socket.remote_endpoint(),
									  asio::make_strand(_ioContext));
	  ...
	  private:
		...
		std::unordered_map<tcp::endpoint, asio::strand<asio::any_io_executor>>
			_accepted;
